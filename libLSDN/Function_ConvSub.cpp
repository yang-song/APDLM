...
template <class N>
ConvSubFunction<N>::ConvSubFunction(const NodeParameters& params) : patchMatrix_(NULL), patchMatrixDiff_(NULL), sz_pm_(NULL), padSize_(params.p), stride_(params.s), numGroup_(params.g), SubsampleH_(params.SubsampleH), SubsampleW_(params.SubsampleW), hasBias_(params.hasBias), hasRELU_(params.hasRELU) {}

template <class N>
ConvSubFunction<N>::~ConvSubFunction() {}

template <class N>
void ConvSubFunction<N>::Clear() {
...
template <class N>
void ConvSubFunction<N>::AccumPatchDiff2Im(i2t<false>, ValueType* img, SizeType numRow, SizeType numCol, SizeType numChan, SizeType kNumRow, SizeType kNumCol) {
	...
	for (SizeType c = 0; c < c_out; ++c) {
		...
		SizeType h_offset = (c % kNumRow)*SubsampleH_;// -(kNumRow - 1)*(SubsampleH_ / 2) + (SubsampleH_>1)*padSize_;
		SizeType w_offset = ((c / kNumRow) % kNumCol)*SubsampleW_;// -(kNumCol - 1)*(SubsampleW_ / 2) + (SubsampleW_>1)*padSize_;
...
template <class N>
void ConvSubFunction<N>::Im2Patches(i2t<false>, ValueType* img, SizeType numRow, SizeType numCol, SizeType numChan, SizeType kNumRow, SizeType kNumCol) {
...
template <class N>
void ConvSubFunction<N>::AdditionModuloOperand(i2t<false>, ValueType* res, SizeType numEl, ValueType* addend, SizeType op_division, SizeType op_modulo) {
...
template <class N>
void ConvSubFunction<N>::BiasDerivativeSingleDim(i2t<false>, ValueType* res, ValueType* input, SizeType patchSize, SizeType numSamples, SizeType numChannels, bool performAddition) {
	if (performAddition) {
...
template <class N>
void ConvSubFunction<N>::BiasDerivativeMultiDim(i2t<false>, ValueType* res, ValueType* input, SizeType sampleSize, SizeType numSamples, bool performAddition) {
	if (performAddition) {
...
template <class N>
void ConvSubFunction<N>::AdjustDimension(TreePostIter& cur) {
...
template <class N>
void ConvSubFunction<N>::Evaluate(TreePostIter& cur, STATE){
	...
	if (NodeType::sz == NULL && NodeType::value == NULL){
		...
		//NodeType::sz[0] = (sz_phi[0] + 2 * padSize_ - sz_w[0] * SubsampleH_ + ((SubsampleH_>1) ? 1 : 0)) / stride_ + 1;
		//NodeType::sz[1] = (sz_phi[1] + 2 * padSize_ - sz_w[1] * SubsampleW_ + ((SubsampleW_>1) ? 1 : 0)) / stride_ + 1;
		NodeType::sz[0] = (sz_phi[0] + 2 * padSize_ - ((sz_w[0]-1)*SubsampleH_ + 1)) / stride_ + 1;
		NodeType::sz[1] = (sz_phi[1] + 2 * padSize_ - ((sz_w[1]-1)*SubsampleW_ + 1)) / stride_ + 1;
		NodeType::sz[2] = sz_w[3];
		NodeType::sz[3] = sz_phi[3];

		//std::cout << "ConvSub: " << sz[0] << " - " << sz[1] << " - " << sz[2] << " - " << sz[3] << std::endl;
...
template <class N>
void ConvSubFunction<N>::Gradient(TreePostIter& cur) {
	assert(cur.LSDN_NUMBER_OF_CHILDREN() > (hasBias_ ? 2ul : 1ul));
	typename NodeType::TreeSiblIter c_b = cur.begin();
	NodeType* c_b_ptr = LSDN_NODE_ACCESSOR(c_b);
...
template <class N>
typename ConvSubFunction<N>::ValueType ConvSubFunction<N>::GetValue(size_t ix, int AccessOffset, int Stride) {
